...
import gl.coopcycle.service.LivraisonService;
...
/**
 * REST controller for managing {@link gl.coopcycle.domain.Livraison}.
 */
@RestController
@RequestMapping("/api")
public class LivraisonResource {
    ...
    public ResponseEntity<LivraisonDTO> updateLivraison(
        ...
        LivraisonDTO result = livraisonService.update(livraisonDTO);
        return ResponseEntity
            .ok()
    ...
    public List<LivraisonDTO> getAllLivraisons(@RequestParam(required = false) String filter) {
        if ("produit-is-null".equals(filter)) {
            log.debug("REST request to get all Livraisons where produit is null");
            return livraisonService.findAllWhereProduitIsNull();
        }
